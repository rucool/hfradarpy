    ...
    *,
    ...
    if scale:
        ...
        u, v = spdir2uv(  # convert angle and speed back to u/v, normalizing the arrow sizes
            np.ones_like(speed), angle, degrees=True
        )
    ...
    if "velocity" in plot_type:
        ...
        kwargs["color_clipped"] = np.clip(tds.velocity.data[::sub], velocity_min, velocity_max).squeeze()
        ...
        if prim_filter:
            tds = ds.squeeze()
        kwargs["color_clipped"] = tds.primary_flag_qc.where(
            tds.primary_flag_qc != 1, other=-1
        ).data  # PRIM == 1 where vectors pass qc
        kwargs["offset"] = TwoSlopeNorm(vmin=-1, vcenter=0, vmax=1)
...
def plot_totals(
    ...
    *,
    ...
    kwargs["color_clipped"] = np.clip(speed[::sub], velocity_min, velocity_max).squeeze()
    kwargs["offset"] = Normalize(vmin=velocity_min, vmax=velocity_max, clip=True)
    kwargs["ticks"] = np.append(np.arange(velocity_min, velocity_max, cbar_step), velocity_max)
    kwargs["extent"] = extent

    plt = plot_common(time, lon, lat, u, v, **kwargs)
