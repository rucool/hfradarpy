function [num, den, z, p] = cheb1(n, r, Wn, ftype, anaflag)
...
%   CHEBY1(N,R,Wn,'stop','s') design analog Chebysev Type I filters.  
%   In this case, Wn can be bigger than 1.0.
%
%   See also CHEB1ORD, CHEBY2, BUTTER, BESSELF, FREQZ, FILTER.
...
%   $Revision: 1.17 $  $Date: 1996/07/25 16:39:30 $
...
if btype == 1		% Lowpass
	[a,b,c,d] = lpA(a,b,c,d,Wn);

elseif btype == 2	% Bandpass
	[a,b,c,d] = lpB(a,b,c,d,Wn,Bw);

elseif btype == 3	% Highpass
	[a,b,c,d] = lpC(a,b,c,d,Wn);

elseif btype == 4	% Bandstop
	[a,b,c,d] = lpD(a,b,c,d,Wn,Bw);
end

% step5: Use Bilinear transformation to find discrete equivalent:
if ~analog,
	[a,b,c,d] = bilin(a,b,c,d,fs);
end
...
% Transform to zero-pole-gain and polynomial forms:
	if nargout == 3
		...
		z = cheb1zros(btype,n,Wn,Bw,analog);
		num = z;
		den = p;
		z = k;
	else % nargout <= 2
		den = poly(a);
		num = cheb1numb(btype,n,Wn,Bw,analog,den,r);
		% num = poly(a-b*c)+(d-1)*den;
	end
end

%---------------------------------
function b = cheb1numb(btype,n,Wn,Bw,analog,den,Rp)
...
function z = cheb1zros(btype,n,Wn,Bw,analog)
