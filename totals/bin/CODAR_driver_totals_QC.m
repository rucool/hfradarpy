...
%% build the time for QC
dtime_step=1/24;
dtime2=dtime-conf.QC.timespan/24:dtime_step:dtime;

%% build the Radial Quality Controlled Structure Array and will write the last
%% radial file from each site to it
Rqc=RADIALstruct(length(conf.Radials.Sites));

% Load Radial Data
fprintf(1, 'Loading Radial Data. \n');
fprintf(1, '---------------------------------------------------------------- \n');

%% loop through each of the stations to load the last 12 hours of data

for ii=1:length(conf.Radials.Sites)
% Construct filenames
F = filenames_standard_filesystem(conf.Radials.BaseDir, conf.Radials.Sites(ii), conf.Radials.Types(ii), dtime2, conf.Radials.MonthFlag, conf.Radials.TypeFlag);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
missingRadials.FileNames = [ Rorig(jj).FileName ];
...
if isempty(Rorig)
    fprintf(1, '**************************************************************** \n');
    fprintf('No radial data for %s %s for the time ending %s ... skipping\n', ...
           conf.Radials.Sites{ii},conf.Radials.Types{ii},datestr(dtime));
    fprintf(1, '**************************************************************** \n');
    continue;
end

%% concatenate the radial data
fprintf('Temporal Concating Radials %s\n',conf.Radials.Sites{ii});
Rcat = temporalConcatRadials(Rorig,0.1,0.1,0.1,0.1);

%% Preallocate the arrays for speed
XStuck=ones(size(Rcat.RadComp));
XGradient=ones(size(Rcat.RadComp));

%% Stuck Value and Temporal Derivative Test
%% ------------------------------------------------------------------------


[r,c]=size(Rcat.RadComp);

for kk=1:r
    XStuck(kk,:)=dataqc_stuckvaluetest(Rcat.RadComp(kk,:),conf.QC.stuck.resolution,conf.QC.stuck.num);
    
    XGradient(kk,:)=temporal_derivative_flag(Rcat.RadComp(kk,:),Rcat.TimeStamp,conf.QC.gradient.threshold);
end

XStuck=logical(XStuck);
XGradient=logical(XGradient);

XFlag=XStuck.*XGradient;
XFlag=logical(XFlag);


Rqc(ii).Type=Rcat.Type;
Rqc(ii).SiteName=Rcat.SiteName;
Rqc(ii).SiteOrigin=Rcat.SiteOrigin;
Rqc(ii).TimeStamp=Rcat.TimeStamp(end);

Rqc(ii).LonLat=Rcat.LonLat(XFlag(:,end),:);
Rqc(ii).RangeBearHead=Rcat.RangeBearHead(XFlag(:,end),:);
Rqc(ii).RadComp=Rcat.RadComp(XFlag(:,end),end);
Rqc(ii).Error=Rcat.Error(XFlag(:,end),end);
Rqc(ii).Flag=Rcat.Flag(XFlag(:,end),end);
Rqc(ii).U=Rcat.U(XFlag(:,end),end);
Rqc(ii).V=Rcat.V(XFlag(:,end),end);
...
Rclean = cleanRadials( Rqc, conf.Radials.MaxRadSpeed );
