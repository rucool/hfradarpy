...
from functions.calc import gridded_index
...
def main(grid, mat_file, save_dir, user_attributes):
    try:
        # load .mat file
    ...
    global_attributes = configs.netcdf_global_attributes(user_attributes, time_string)
...
if __name__ == '__main__':
    ...
    # Define test inputs
    files = sorted(glob.glob(cf.path_within_module('data/totals/mat/*.mat')))
    grid_file = cf.path_within_module('totals/grid_files/OI_6km_Grid_Extend.txt')
    save_dir = cf.path_within_module('data/totals/nc')
    ...
    for file in files:
        print file
        main(grid, file, save_dir, user_attributes)
